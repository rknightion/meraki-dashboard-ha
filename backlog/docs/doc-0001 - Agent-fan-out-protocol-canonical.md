...
> authoritative for `meraki-dashboard-ha`: an agent with only this checkout has the whole model,
> with nothing to fetch from a machine-local file. Everything below the rule is the source
> unchanged — not a summary, excerpt or adaptation. When the source changes, the edit is not
