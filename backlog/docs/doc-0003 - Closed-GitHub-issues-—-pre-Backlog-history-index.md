...
**Not on GitHub.** The issues authored by the repository owner and its bots were deleted on
2026-08-14, immediately after this archive was captured and pushed. `gh issue view <N>` will 404 for
those. The record is `archive/github-issues-2026-08-14.json`, which is **redacted** — Meraki
identifiers, reporter handles and one reporter's Home Assistant system-health dump were replaced with
stable placeholders. See `archive/README.md` for the mapping and for what was deliberately left in.
...
Issues filed by external contributors were **not** deleted and still exist on GitHub. The tracker
remains open for new reports; it is no longer where this project's own work is planned.
...
| --- | --- | --- | --- | --- |
| 50 | Dependency Dashboard | closed | 2026-03-16 | Renovate bot artifact; superseded by #256 |
| 132 | API Permissions | closed | 2025-10-12 | No code change — answered: only org-level API keys are supported, network-level keys 404 |
| 143 | MT20 Door Sensors | closed | 2025-10-17 | `c93df09` fix(sensors): correct MT device model capabilities mapping |
| 144 | MT20 is a door sensor | closed | 2025-10-18 | `c93df09`, `9255697` fix(sensors): expand fallback capabilities |
| 218 | CW9171I not mapping | closed | 2025-12-20 | `8b0491a` feat: add CW device support · `90e7bc2` feat: add device type mapping system · `3dd91c7` refactor: use centralized device type detection · `270c5bb` test: add CW wireless device type tests |
| 256 | Dependency Dashboard | open | — | Renovate bot artifact; left in place and left open |
...
16 issues, 15 closed, 1 open. The epic #295 and its six lane issues #296–#301 all landed as the
single squashed commit `a820159`; the lane split existed for the campaign, not in the history.
