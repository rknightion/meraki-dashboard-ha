...
class TestHandleAPIErrors:
    ...
    async def test_handle_401_error(self, api_error_401):
        ...
        @handle_api_errors(reraise_on=(APIError,))
        async def api_call():
            raise api_error_401
    ...
    async def test_handle_non_api_error(self):
        ...
        @handle_api_errors(reraise_on=(ValueError,))
        async def api_call():
...
class TestShouldRetryError:
    ...
    def test_should_retry_500_error(self, api_error_500):
        ...
        result = should_retry(api_error_500, config, attempt=1)
    ...
    def test_should_not_retry_401_error(self, api_error_401):
        ...
        config = RetryStrategies.DEFAULT
        result = should_retry(api_error_401, config, attempt=1)
    ...
    def test_should_retry_429_error(self, api_error_429):
        ...
        config = RetryStrategies.DEFAULT
        result = should_retry(api_error_429, config, attempt=1)
    ...
    def test_should_not_retry_403_error(self, api_error_403):
        ...
        config = RetryStrategies.DEFAULT
        result = should_retry(api_error_403, config, attempt=1)
...
class TestRetryDelay:
    ...
    def test_delay_increases(self):
        ...
        config = RetryStrategies.DEFAULT
        delays = [calculate_retry_delay(attempt=i, config=config) for i in range(5)]
    ...
    def test_delay_initial_value(self):
        ...
        config = RetryStrategies.DEFAULT
        first_delay = calculate_retry_delay(attempt=0, config=config)
    ...
    def test_delay_max_value(self):
        ...
        max_delay = calculate_retry_delay(attempt=100, config=config)
    ...
    def test_delay_with_different_attempts(self):
        ...
        for attempt in [0, 1, 2, 5, 10]:
            delays[attempt] = calculate_retry_delay(attempt=attempt, config=config)
...
class TestWithStandardRetries:
    ...
    async def test_retry_on_500_error(self):
        """Test retry behavior on timeout error (which is retryable)."""
        call_count = 0
