...
class MerakiDashboardHub:
    ...
    async def async_setup(self) -> bool:
        ...
        try:
            ...
            _LOGGER.error("Unexpected error connecting to Meraki Dashboard API: %s", err)
            raise ConfigEntryNotReady from err
    ...
    async def async_get_devices_by_type(self, device_type: str) -> list[dict[str, Any]]:
        ...
        for network in self.networks:
            try:
                ...
                _LOGGER.error(
                    "Error getting devices for network %s: %s",
                    network["name"],
                    err
                )
    ...
    async def async_get_sensor_data_batch(self, serials: list[str]) -> dict[str, dict[str, Any]]:
        ...
        try:
            ...
            _LOGGER.debug("Received %d readings from API", len(all_readings) if all_readings else 0)
            ...
            if network_id:
                try:
                    device = await self.hass.async_add_executor_job(
                        self.dashboard.devices.getNetworkDevice,
                        network_id,
                        serial
                    )
